...
class RBAC {
    ...
    static function saveAccessList($authId=null) {
        RBAC::getAccessLists($authId);
        if(null===$authId)   $authId = $_SESSION[C('USER_AUTH_KEY')];
        // 如果使用普通权限模式，保存当前用户的访问权限列表
        // 对管理员开发所有权限
        if(C('USER_AUTH_TYPE') !=2 && !$_SESSION[C('ADMIN_AUTH_KEY')] )
            $_SESSION['_ACCESS_LIST']	=	RBAC::getAccessLists($authId);
        return ;
    }
        ...
        if(RBAC::checkAccess()) {
            ...
            if(empty($_SESSION[C('ADMIN_AUTH_KEY')])) {
                if(C('USER_AUTH_TYPE')==2) {
                    ...
                    //通过数据库进行访问检查
                    $accessList = RBAC::getAccessLists($_SESSION[C('USER_AUTH_KEY')]);
                }else {
   ...
   static public function getAccessLists($authId){
       header("Content-type:text/html;charset=utf-8");
       //dump($authId);exit;
       // Db方式权限数据
       $db      =  Db::getInstance(C('RBAC_DB_DSN'));
       $table = array('role'=>C('RBAC_ROLE_TABLE'),'role_user'=>C('RBAC_USER_TABLE'),'access'=>C('RBAC_ACCESS_TABLE'),'node'=>C('RBAC_NODE_TABLE'));
       $sql    =   "select node.id,node.name,node.title from ".
           $table['role']." as role,".
           $table['role_user']." as role_user,".
           $table['access']." as access ,".
           $table['node']." as node ".
           "where role_user.user_id='{$authId}' and role_user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id  and node.pid<>0";
       $apps =   $db->query($sql);  
       //echo $db->getLastSql();
       //dump($apps);exit;
       $access_list = array();
       ...
       if($authId==1){
           $Node = D('Node');
           $node_list = $Node->getField('name',true);
           foreach ($node_list as $k=>$v){
               $node_list[$k] = strtolower($v);
           }
       }
       
       $access_list = array_unique(array_merge(array_filter($access_list),array('index/index')));
...
//       dump($action);
//       dump($access_list);
//        dump(in_array($model.'/'.$action, $access_list));
//        exit;
       
       if(in_array($model.'/'.$action, $access_list) || $authId==1){
           return true;
       }
       else{
           return false;
